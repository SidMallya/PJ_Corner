...
$post_query = "SELECT * FROM pj ORDER BY pj_rating DESC LIMIT ".$starting_post.",".$posts_per_page;
		...
		<form name="jump1">
           <select name="myjumpbox" OnChange="location.href=jump1.myjumpbox.options[selectedIndex].value">
			  ...
			  for($pn = 1; $pn <= $total_pages_integer; $pn++) {
                 echo '<option '.select_current_page($page, $pn).' value="index.php?content=top_rated&pn='.$pn.'">'.$pn;
			  }
			  ?>
 		   </select>
		</form>		
